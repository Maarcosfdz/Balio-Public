...
-- ─────────────────────────────────────────────────────────────
-- Scheduled Transactions: recurring transaction rules
-- ─────────────────────────────────────────────────────────────
...
CREATE TABLE scheduled_transactions (
    ...
    freq_years       INT          NOT NULL DEFAULT 0,
    freq_months      INT          NOT NULL DEFAULT 0,
    freq_weeks       INT          NOT NULL DEFAULT 0,
    freq_days        INT          NOT NULL DEFAULT 0,
    start_date       DATE         NOT NULL,
    last_execution   DATE,
    ...
    CHECK (freq_years >= 0 AND freq_months >= 0 AND freq_weeks >= 0 AND freq_days >= 0),
    CHECK (freq_years + freq_months + freq_weeks + freq_days > 0),

    CONSTRAINT fk_st_user
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    CONSTRAINT fk_st_account
        FOREIGN KEY (account_id) REFERENCES accounts(id) ON DELETE SET NULL,
    CONSTRAINT fk_st_category
        FOREIGN KEY (category_id) REFERENCES categories(id) ON DELETE SET NULL
);

CREATE INDEX ix_scheduled_transactions_user ON scheduled_transactions(user_id);
