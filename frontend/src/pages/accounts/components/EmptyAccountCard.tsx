...
export default function EmptyAccountCard({ onAdd, currentCount }: EmptyAccountCardProps) {
  ...
  return (
    <button
      ...
      disabled={isFull}
      className={`flex flex-col items-center justify-center gap-4 rounded-2xl border-2 border-dashed py-12 transition ${
        isFull
          ? "border-slate-100 bg-slate-50 text-slate-300 cursor-not-allowed"
          : "border-slate-200 bg-white text-slate-400 hover:border-sky-300 hover:bg-sky-50/50 hover:text-sky-500"
      }`}
    >
      <div className={`flex h-14 w-14 items-center justify-center rounded-full border-2 border-dashed border-current ${isFull ? 'bg-slate-100' : ''}`}>
